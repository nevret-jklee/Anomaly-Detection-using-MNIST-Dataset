...
inspect.getfile(inspect.currentframe()) : 현재 파일이 위치한 경로 + 현재 파일 명
os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe()))) : 현재 파일이 위치한 경로
'''
...
def boxplot(contents, savename=""):
    ...
    plt.tight_layout()
    plt.savefig(savename)
    plt.close()   # Close the figure window entirely
    ...
    for epoch in range(epochs):
        ...
        if (neuralnet.z_dim == 2):
            ...
            try:
                ...
                latent_plot(latent=pca_features, 
                            y=y_tr, 
                            n=dataset.num_class,
                            savename=os.path.join("results", "tr_latent", "%08d.png" %(epoch)))
            except: 
            ...
            for y_loc, y_val in enumerate(y_values):
                for x_loc, x_val in enumerate(x_values):
                    ...
                    if (z_latents is None): 
                        ...
                        z_latents = np.append(z_latents, z_latent, axis=0)
        ...
        print("Epoch [%d / %d] (%d iteration)  Restore:%.3f, KLD:%.3f, Total:%.3f" % (epoch, epochs, iteration, restore_error, kl_divergence, tot_loss))
...
def test(neuralnet, dataset):
    ...
    while (True):
        ...
        tot_loss, restore_error, kl_divergence = loss_functions(x=x_te_torch, 
                                                                x_hat=x_hat, 
                                                                mu=z_mu, 
                                                                sigma=z_sigma)
        score_anomaly = restore_error.item()
